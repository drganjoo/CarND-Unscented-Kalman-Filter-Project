...
n_aug_(7)
{
    ...
    // initialize Sigma Predicted matrices
    ...
    // Since laser is a linear process we won't be using sigma points for that
    // initialize laser related matrices here
...
void UKF::ProcessMeasurement(const Radar &radar)
{
    if (!is_initialized_) {
        ...
        PredictState(radar.timestamp);
        
        MatrixXd radar_sig, radar_S;
        VectorXd radar_pred;
        
        // transform sigma points from the state space into the radar measurement
        // space so that the incoming radar measurement can be compared with the
        // sigma points
        
        TransformSigmaToRadar(&radar_sig, &radar_pred, &radar_S);
        
        // update state based on the incoming measurement
...
void UKF::ProcessMeasurement(const Lidar &lidar)
{
    if (!is_initialized_) {
        ...
        PredictState(lidar.timestamp);

        // use linear process to update state in case of lidar
...
void UKF::PredictState(long long measurement_time_us)
{
    double delta_t_secs = (measurement_time_us - time_us_) / 1000000.0;
    time_us_ = measurement_time_us;
    
    // During debug, make sure we haven't missed out any measurement
...
{
    const double delta_t2 = delta_t * delta_t; // square of delta_2
...
{
    // x = ∑(wi * X_sigma)
    // In Eigen this can be achieved by using the dot produce between
    // XSigma and weights
    
    x_ = Xsig_pred_ * weights_;    // this will multiply wi with Xsig_pred and sum it as well
    
    // P = ∑(wi(X − x)(X − x)T
    // In eigen this can be acheived by first doing (X-x) * weights and then * (X-x)T
                                ...
                                MatrixXd *S)
{
    ...
    // create matrices for converting sigma points into measurement space
    ...
    *z_pred = *Zsig * weights_;
    ...
    // In eigen, instead of doing Zsig - z_pred we can do
    // (-Zig) + z_pred
...
void UKF::Update(const VectorXd &z, const MatrixXd &Zsig, const VectorXd &z_pred, const MatrixXd &S, bool normalize /*= true*/)
{
    ...
    // compute NIS and save it for this run
